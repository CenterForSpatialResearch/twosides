...
//
// Options 1-5 are five passes over ONE arrangement, newest first. They share a
...
// narrative0821() / refined0821() instead. Options 6-9 are biomes-only arrangements, and the
...
export const UI_OPTIONS = [
  'final ui', // 1 — the country-from-map pass with the final copy: the
  //                  lifestyle split is named Westernized / Non-Westernized
  //                  everywhere (info panel, row titles), as it is on the
  //                  species stat line
  'country from map', // 2 — the narrative pass, with the map's click target
  ...
  '8-21 narrative updates', // 3 — the 8/21 refinements plus the exhibit copy:
  ...
  'ui refinements 8/21', // 4 — the 8/14 arrangement, refined: the anthrome
  ...
  //                  word they call out
  'ui refinements 8/14', // 5 — arced control labels on both sides; biomes
  ...
  'Lifestyle', // 6 — Western / Non-Western country rows, ranked by % unknown;
  //                  leaves recolour white/magenta when a country is selected
  'Country', //  7 — country-first: CountryCircle picker + per-country breakdown
  'Split', //    8 — Known/Unknown and Non/Western share a row
  'Prevalence', // 9 — details on top, then Prevalence + Known/Unknown stacked
];
...
// True for ALL FIVE refinement passes (1 = final ui, 2 = country-from-map,
// 3 = narrative, 4 = 8/21, 5 = 8/14). They are one arrangement, so every layout
...
// True only for the newest pass: the final copy. Identical to country-from-map

...
```bash
cd processing/
python3 5_smooth_boundaries.py                       # both shipped sets
node 2_generate_topojson.js \
  --input=geojson/admin-boundaries/smooth \
  --output=../public/topojson/admin-boundaries \
  --quantization=1e5 --only=countries-110m,countries-50m
```

`5_smooth_boundaries.py` prints the exact `2_generate_topojson.js` invocations it needs, grouped by simplification, so copy them from its output rather than from here if you change the set list.

**Output**: `public/topojson/admin-boundaries/countries-110m.topojson` (103 KB) and `countries-50m.topojson` (727 KB).

**Source data**: Natural Earth Admin-0 Countries shapefiles at `data/ne_110m_admin_0_countries/` and `data/ne_50m_admin_0_countries/`.

Regenerating 110m rewrites it with a byte-identical file, so an empty diff there is the expected result rather than a sign nothing ran.

### Country sets

110m is first-order countries only — no territories, so the entire Lesser Antilles is missing and the Caribbean draws no country lines. The app offers a second set, and the DevHud's **BOUNDARIES** button on the anthromes page switches between them:

| set | source | id field | table | encoding | cells attributed @70km |
|---|---|---|---|---|---|
| `110m` | `ne_110m_admin_0_countries` | `ISO_A3_EH` | 175 | uint8 | 171 |
| `50m` | `ne_50m_admin_0_countries` | `ADM0_A3` | 243 | uint8 | 204 |

A land cell's country is one index into that table, so a set is free until the table passes 256 entries, at which point `countries-<set>.bin` doubles in width. 50m carries every dependency and still fits, which is why it is the one that ships. `ADM0_A3` rather than `ISO_A3_EH` also recovers Kosovo, Somaliland and N. Cyprus, which an ISO3 join drops.

Two further sets stay defined in `country_sets.py` but are not built by default and not committed, because both would widen every profile's blob to uint16 — `10m` (258 entries, for the sake of three past the byte) and `10m-units` (298; splits France into its overseas departments, but also the UK into its four nations and Bosnia into three). Build them with `--sets=` / `--set=` to compare.

...
**Windows (PowerShell):**
```powershell
pip install rasterio shapely numpy pyshp
```
...
## Admin Boundaries (BOUNDARY-SNAP)

Grid-snapped country boundaries that align with anthrome cell boundaries. These are generated by rasterizing Natural Earth vector boundaries to the same grid resolution as the anthromes data, then re-vectorizing.

### Source Data

Download Natural Earth Admin-0 Countries (110m resolution) from [naturalearthdata.com](https://www.naturalearthdata.com/downloads/110m-cultural-vectors/110m-admin-0-countries/) and extract to `data/ne_110m_admin_0_countries/`.
...
### Example Boundary Profiles

Run commands from `processing/`. Output goes to `public/topojson/admin-boundaries/{profile}/`.
...
- **Blocky Boundaries**: The rasterize/vectorize process creates "stair-step" boundaries at the grid resolution. This is intentional for alignment with anthrome cells.
- **Small Countries**: At coarse resolutions (50km), small island nations may disappear entirely or be reduced to a single cell.
- **Object Naming**: The TopoJSON object is named `anthromes` (reusing existing script) but contains boundary data. The viz loads by object index, not name.
